...
exports.getTotalScore = function(req,res) {
    ...
    pool.getConnection(function(err, connection){
        ...
        connection.query("CALL spGetTotalScore(" + connection.escape(req.body.user_team_uuid) + ");", function(err, rows){          
            connection.release();            
            if(!err) {                                
                var response = JSON.stringify(rows[0]); 
                //console.log("Response res - " + response);
...
exports.getCurrentTimeSnapshot = function(req,res) {
    ...
    pool.getConnection(function(err, connection){
        ...
        connection.query("CALL spGetCurrentTimeSnapshot(" + connection.escape(req.body.user_team_uuid) + ");", function(err, rows){          
            connection.release();            
            if(!err) {                                
                var response = JSON.stringify(rows[0]); 
                //console.log("Response res - " + response);
